      ...
      <AnimatePresence initial={false}>
        {view === "list" && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="flex-1 flex flex-col min-h-0 w-full pt-0 pb-3"
            style={{ paddingTop: "0px", paddingBottom: "12px" }}
          >
            {/* Navigation removed as per simplification request */}
            ...
            {activeTab !== "BulkPayment" && activeTab !== "Pivot" && (
              <motion.div
                ...
                className="flex-1 flex flex-col min-h-0 w-full pb-0 pt-0"
                style={{ paddingTop: "0px", paddingBottom: "0px", paddingLeft: "16px", paddingRight: "16px" }}
              >
                <div 
                  className="bg-card text-card-foreground soft-card force-light flex-1 flex flex-col min-h-0 relative z-10 w-full overflow-hidden border border-border shadow-md rounded-[16px] master-ae-table-wrapper" 
                  style={{ paddingBottom: "0px", paddingTop: "0px", borderRadius: "0px", borderWidth: "0.5px" }}
                >`;
...
const oldHeaderRegex = /\{\/\* Header area - Simplified \*\/\}[\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*\{\/\* Sibling 2 of Card: Striped pattern \*\/\}/m;
code = code.replace(oldHeaderRegex, `              {/* Sibling 2 of Card: Striped pattern */}`);

// We need to fix closing tags at the bottom.
// In the old layout, the tables were closed by:
/*
              </div>
            </div>
            </div>
            <ConfirmDialog
*/
// Let's replace the last 3 `</div>` with the new closing tags needed.
// We added the Right Panel which requires 2 extra closing divs.
// Wait, the structure we added replaced:
/*
              <motion.div
                key="list-tables"
...
...
...
            {activeTab !== "BulkPayment" && activeTab !== "Pivot" && (
*/
// So the number of open divs remains the same! We opened `grid`, `flex-col`, `table-container` instead of `motion.div`, `div`. Actually 3 open divs instead of 2.
// Wait, no. The new code opens:
/*
            <div className={\`flex-1 grid ... \`}>
              ...
              <div className="flex-1 flex flex-col min-h-0 h-full overflow-hidden relative animate-in fade-in slide-in-from-right duration-500 content-area">
                <div className="table-container flex-1 flex flex-col min-h-0 relative bg-card border border-border rounded-xl shadow-sm overflow-hidden master-ae-table-wrapper">
                   ...
                   {activeTab === "BulkPayment" && ...}
                   {activeTab === "Pivot" && ...}
                   {activeTab !== "BulkPayment" && activeTab !== "Pivot" && (
                       ...
*/
// Wait, `BulkPayment` and `Pivot` are now INSIDE `table-container`.
// In the original, they were OUTSIDE `motion.div`.
// We need to fix this.

// Wait, let's just write the end of the file.
const endOfTablesRegex = /\{\/\* Sibling 2 of Card: Striped pattern \*\/\}[\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*<ConfirmDialog/m;
const oldTablesSection = code.match(endOfTablesRegex)[0];

const newTablesSection = oldTablesSection
    .replace('</div>\n            </div>\n            </div>\n            <ConfirmDialog', '</div>\n              )} <!-- closes activeTab !== BulkPayment -->\n              </div>\n              </div>\n            </div>\n          </motion.div>\n        )}\n      </AnimatePresence>\n      <ConfirmDialog');

// Actually it's easier to just do string manipulation or manual fix via edit_file if this script fails. Let's just output the modified code to a file and see.

fs.writeFileSync('scripts/tmp-masterae-replace.js', code);
