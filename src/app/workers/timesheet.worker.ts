...
export function calculateTimesheet(params: any) {
  ...
  const finalize = (groupObj: Record<string, any>) => Object.values(groupObj).map((row: any, index) => {
    ...
    if (isMktLocal) { 
      ...
      chargeOther = Math.round(((row.other || 0) + (row.moth01 || 0)) * 20000);
      ...
      cMktLocal = Math.round(((row.supportMkt || 0) + (row.lpar01 || 0) + (row.lret01 || 0) + (row.ldem01 || 0) + (row.ldec01 || 0)) * 20000);

      // Charge LXO (Trừ lùi)
      chargeLxo = totalSalary - cEc - cPtDemo - chargeOther - cRenewal - cDiscovery - cSummerOuting - cSummerInstructors - cMktLocal;
